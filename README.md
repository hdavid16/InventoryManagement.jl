...
- Backlogging is allowed at all nodes. When `reallocation = true`, the unfulfilled quantity from the lowest priority supplier is added to the highest priority supplier request in the next period. *Note:* backlogging is only penalized at the markets currently. Unfulfilled replenishment orders can also be penalized by adding a penalty parameter at each node (future work).
